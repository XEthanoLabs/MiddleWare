...
using ip::tcp;
    ...
    void Server::AsyncAllowClientToJoin()
    {
        m_pAcceptor->async_accept([this](boost::system::error_code ec, tcp::socket sk)
            {
                if (!ec)
                {
                    ...
                    shared_ptr<ConnectedClient> pCC = make_shared<ConnectedClient>(s    k, this); // you HAVE to use move
    ...
    void Server::SendMessagesToClientsLoop()
    {
        // m_pIoService->run();
    ...
    void Server::IncomingSetClientName(string& szClient, tcp::socket& s )
    {
    ...
    void Server::IncomingCreateTopic(string& szClient, string& szTopicName)
    {
    ...
    TopicRoom* Server::FindTopicRoom(string& szTopicName)
    {
    ...
    void Server::IncomingSubscribeTopic(string& szClient, string& szTopicName)
    {
    ...
    void Server::IncomingUnsubscribeTopic(string& szClient, string& szTopicName)
    {
        shared_ptr<ConnectedClient>& pCC = GetConnectedClientFromName(szClient);
        if (pCC == nullptr)
        {
    ...
    void Server::IncomingMessage(char chPriority, string& szClient, string& szTopicName, string& szMesg)
    {
        TopicRoom* ptr = FindTopicRoom(szTopicName);
        if (ptr == nullptr)
        {
    ...
    shared_ptr<ConnectedClient>& Server::GetConnectedClientFromName(string& szClient)
    {
        for (shared_ptr<ConnectedClient>& cc : m_Clients)
        {
    ...
    shared_ptr<ConnectedClient>& Server::GetConnectedClientFromSocket(tcp::socket& socket)
    {
        for (shared_ptr<ConnectedClient>& cc : m_Clients)
        {
    ...
    void Server::_ProcessSingleCommand(string& szSingleLine, tcp::socket& socket)
    {
    ...
    void Server::OnData(string& szMesg, tcp::socket& socket)
    {
        while (szMesg.size())
        {
            string szFirst;
    ...
    void Server::OnSocketClose(tcp::socket& socket)
    {
